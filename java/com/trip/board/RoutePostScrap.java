...
/**
 * 경로 게시물 스크랩 기능을 처리하는 서블릿
 * @author jsg
 * @version 1.0
 * @since 2025.10.24
 */
@WebServlet("/routepost/scrap.do")
public class RoutePostScrap extends HttpServlet {

    /**
     * GET 요청을 처리하여 현재 로그인한 사용자의 특정 경로 게시물(id)에 대한
     * 스크랩 상태를 토글(추가/취소)합니다.
     * 로그인 상태가 아니면 에러 메시지를 JSON으로 반환합니다.
     * 로그인 상태이면 DB에서 스크랩 상태를 토글(toggleScrap)하고,
     * 처리 결과(성공 여부, 스크랩 상태, 메시지)를 JSON 형식으로 응답합니다.
     *
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        boolean scrapped = dao.toggleScrap(userId, postId);
        dao.close();

        obj.put("success", true);
        obj.put("scrapped", scrapped);
        obj.put("message", scrapped ? "스크랩했습니다 📌" : "스크랩을 취소했습니다 ❌");
