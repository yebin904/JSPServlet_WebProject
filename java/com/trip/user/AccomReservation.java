...
/**
 * 숙소 예약 목록을 조회하고 페이징을 처리하는 서블릿
 * (마이페이지 - 내 숙소 예약 내역)
 * @author jsg
 * @version 1.0
 * @since 2025.10.24
 */
@WebServlet(value = "/user/accomreservation.do")
public class AccomReservation extends HttpServlet {
	 ...
	 * 4. `AccomReservationDAO`를 통해 해당 사용자의 총 숙소 예약 건수(`totalCount`)를 조회합니다.
	 * 5. `totalCount`를 기반으로 총 페이지 수(`totalPage`)를 계산합니다.
	 * 6. `AccomReservationDAO`를 통해 현재 페이지에 해당하는 숙소 예약 목록(`list`)을 조회합니다.
	 * 7. (주석 처리됨: 데이터 가공 로직 - 날짜 형식 변경, 제목 자르기 등)
	 * 8. 페이지 번호를 기반으로 페이지바(HTML)를 생성합니다.
	 * 9. 조회된 목록(`list`), 페이징 정보(`map`), 페이지바(`pagebar`)를 request 속성에 저장합니다.
	 * 10. 숙소 예약 목록 페이지(`/WEB-INF/views/user/accomreservation.jsp`)로 포워딩합니다.
	 *
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//총 게시물 수?
		totalCount = dao.getAccomResrvationTotalCount(map);
		...
		List<AccomReservationDTO> list = dao.totalAccomList(map);
		 ...
		 * * //날짜 조작 > 오늘 날짜? String regdate = dto.getRegdate();
		 ...
		 * * * //제목 자르기 String subject = dto.getSubject(); if (subject.length() > 15) {
		 * subject = subject.substring(0, 15) + ".."; }
		 * * //태그 비활성화 subject = subject.replace("<", "&lt;").replace(">", "&gt;");
		...
		*/
		...
		if (n == 1) {
			...
			pagebar += String.format(" <a href='/trip/user/accomreservation.do?page=%d'>이전</a> ", n - 1, blockSize);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				pagebar += String.format(" <a href='/trip/user/accomreservation.do?page=%d' class='page'>%d</a> ", n, n);
			}
		...
		if (n >= totalPage) {
			...
			pagebar += String.format(" <a href='/trip/user/accomreservation.do?page=%d'>다음</a> ", n, blockSize);
		}
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/user/accomreservation.jsp");
