...
/**
 * 사용자 스크랩 활동 내역을 조회하고 페이징을 처리하는 서블릿
 * (마이페이지 - 내 스크랩 목록)
 * @author jsg
 * @version 1.0
 * @since 2025.10.24
 */
@WebServlet(value = "/user/scrapactivities.do")
public class ScrapActivities extends HttpServlet {
	 ...
	 * 4. `BoardDAO`를 통해 해당 사용자가 스크랩한 총 게시물 수(`totalCount`)를 조회합니다.
	 * 5. `totalCount`를 기반으로 총 페이지 수(`totalPage`)를 계산합니다.
	 * 6. `BoardDAO`를 통해 현재 페이지에 해당하는 스크랩 게시물 목록(`list`)을 조회합니다.
	 ...
	 * 10. 스크랩 활동 내역 페이지(`/WEB-INF/views/user/scrapactivities.jsp`)로 포워딩합니다.
	 * (참고: 페이지바 링크 URL에 'scarpactivities.do'라는 오타가 있으나 원본 유지를 위해 수정하지 않습니다.)
	 *
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//총 게시물 수?
		totalCount = dao.getAllScrapTotalCount(map);
		...
		java.util.List<BoardDTO> list = dao.totalScrapList(map);
		...
		*/
		...
		if (n == 1) {
			...
			pagebar += String.format(" <a href='/trip/user/scarpactivities.do?page=%d'>이전</a> ", n - 1, blockSize);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				pagebar += String.format(" <a href='/trip/user/scarpactivities.do?page=%d' class='page'>%d</a> ", n, n);
			}
		...
		if (n >= totalPage) {
			...
			pagebar += String.format(" <a href='/trip/user/scarpactivities.do?page=%d'>다음</a> ", n, blockSize);
		}
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/user/scrapactivities.jsp");
