...
/**
 * 렌터카 예약 목록을 조회하고 페이징을 처리하는 서블릿
 * (마이페이지 - 내 렌터카 예약 내역)
 * @author jsg
 * @version 1.0
 * @since 2025.10.24
 */
@WebServlet(value = "/user/carreservation.do")
public class CarReservation extends HttpServlet {
	 ...
	 * 4. `CarReservationDAO`를 통해 해당 사용자의 총 렌터카 예약 건수(`totalCount`)를 조회합니다.
	 * 5. `totalCount`를 기반으로 총 페이지 수(`totalPage`)를 계산합니다.
	 * 6. `CarReservationDAO`를 통해 현재 페이지에 해당하는 렌터카 예약 목록(`list`)을 조회합니다.
	 ...
	 * 10. 렌터카 예약 목록 페이지(`/WEB-INF/views/user/carreservation.jsp`)로 포워딩합니다.
	 *
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//총 게시물 수?
		totalCount = dao.getCarResrvationTotalCount(map);
		...
		List<CarReservationDTO> list = dao.totalCarList(map);
		...
		*/
		...
		if (n == 1) {
			...
			pagebar += String.format(" <a href='/trip/user/carreservation.do?page=%d'>이전</a> ", n - 1, blockSize);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				pagebar += String.format(" <a href='/trip/user/carreservation.do?page=%d' class='page'>%d</a> ", n, n);
			}
		...
		if (n >= totalPage) {
			...
			pagebar += String.format(" <a href='/trip/user/carreservation.do?page=%d'>다음</a> ", n, blockSize);
		}
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/user/carreservation.jsp");
