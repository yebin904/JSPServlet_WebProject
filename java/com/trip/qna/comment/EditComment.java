...
/**
 * Q&A 게시물 댓글 수정을 처리하는 서블릿
 * @author jsg
 * @version 1.0
 * @since 2025.10.24
 */
@WebServlet("/trip/qna/comment/edit.do")
public class EditComment extends HttpServlet {
    ...
    /**
     * POST 요청을 처리하여 Q&A 게시물의 댓글을 수정합니다.
     * 세션에서 사용자 ID와 관리자 권한을 확인하고, 댓글 ID와 수정 내용을 받아 데이터베이스에 업데이트합니다.
     * 수정 성공 시 게시물 상세 페이지로 리다이렉트하고, 실패 시 에러 메시지를 표시합니다.
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean success = dao.editComment(commentId, sessionUserId, content, isAdmin);
        dao.close();
        ...
        if (success) {
            ...
            response.getWriter().println("<script>alert('댓글 수정 실패'); history.back();</script>");
        }
    }
