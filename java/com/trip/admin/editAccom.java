...
/**
 * 관리자용 숙소 수정을 처리하는 서블릿
 * @author jsg
 * @version 1.0
 * @since 2025.10.24
 */
@WebServlet("/admin/accom/edit.do")
public class editAccom extends HttpServlet {

    /**
     * 수정할 숙소 정보를 조회하여 수정 폼 페이지로 이동합니다.
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        accomAllInfoDTO dto = dao.getAccommodationInfo(roomId);
        
        req.setAttribute("dto", dto);
        
        RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/admin/accomedit.jsp");
        dispatcher.forward(req, resp);
    }

    /**
     * 폼에서 수정된 숙소 정보를 DB에 저장합니다.
     * @param req HttpServletRequest 객체
     * @param resp HttpServletResponse 객체
     * @throws ServletException 서블릿 예외
     * @throws IOException 입출력 예외
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        dto.setPlaceId(Integer.parseInt(req.getParameter("placeId")));
        dto.setRoomId(Integer.parseInt(req.getParameter("roomId")));
        ...
        accomDAO dao = new accomDAO();
        int result = dao.updateAccommodation(dto);

        if (result == 1) {
            resp.sendRedirect(req.getContextPath() + "/admin/accom/list.do");
        } else {
            resp.getWriter().write("<script>alert('숙소 수정에 실패했습니다.'); history.back();</script>");
        }
    }
}